 ...
 * baseUrl is `http://localhost:11434`. The caller passes their own
 * via `config.baseUrl`; the relay passes it through `config.apiKey`
 * (the playground's BYOK slot for Ollama is `kind: 'baseUrl'`; its
 * `getKey()` returns the user-configured URL or the default), so the
 * provider reads `config.baseUrl ?? config.apiKey`.
 *
...
function resolveEndpoint(rawBaseUrl: string | undefined): string {
  // The playground passes the baseUrl through config.apiKey; direct
  // callers pass config.baseUrl. Trim trailing slashes so
  // `/v1/chat/completions` concatenates cleanly. Fall back to the
  // documented default when the slot is empty or not an http(s) URL.
  const baseUrl =
...
/**
 * Build an Ollama `ModelClient`. The Ollama server base URL comes from
 * `config.baseUrl ?? config.apiKey` (relay routes it via apiKey;
 * direct callers pass baseUrl); per-call values (messages, tools,
 * sampling) come in the `ModelRequest`.
 */
export function ollamaModelClient(config: OllamaConfig): ModelClient {
  const endpoint = resolveEndpoint(config.baseUrl ?? config.apiKey);
  return {
