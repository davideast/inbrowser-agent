...
describe('createReactLoopStrategy', () => {
  test('completes a single text turn with no tool calls', async () => {
    ...
    const llm = fakeLlm([[
      ...
      { kind: 'turn_complete', usage: { promptTokens: 10, completionTokens: 1 }, details: { requestedModel: 'fake' } },
    ]]);
    const events = await collect(strategy.run({
      prompt: 'hi',
      history: [],
      workspace: EMPTY_WORKSPACE,
      runtime: EMPTY_RUNTIME,
  ...
  test('drives a tool-call → result → next-turn loop', async () => {
    const echoTool: ToolHandler<{ msg: string }, { msg: string }> = {
      ...
      async execute({ msg }) { return { ok: true, summary: msg, data: { msg } }; },
    };
    ...
    const llm = fakeLlm([
      [
        ...
        { kind: 'turn_complete', usage: { promptTokens: 5, completionTokens: 5 }, details: { requestedModel: 'fake' } },
      ],
      [
        { kind: 'text', chunk: 'Tool said: hi from tool' },
        { kind: 'turn_complete', usage: { promptTokens: 15, completionTokens: 4 }, details: { requestedModel: 'fake' } },
      ],
    ]);

    const events = await collect(createReactLoopStrategy().run({
      prompt: 'use the tool',
      history: [],
      workspace: EMPTY_WORKSPACE,
      runtime: EMPTY_RUNTIME,
  ...
  test('caps runaway loops via maxTurns', async () => {
    ...
    const looperLlm: LlmClient = {
      ...
      chat(): AsyncIterable<ChatEvent> {
        return (async function* () {
          ...
          yield { kind: 'turn_complete', usage: { promptTokens: 1, completionTokens: 1 }, details: { requestedModel: 'fake' } };
        })();
      },
    };
    ...
    const events = await collect(createReactLoopStrategy({ maxTurns: 3 }).run({
      prompt: 'x',
      history: [],
      workspace: EMPTY_WORKSPACE,
      runtime: EMPTY_RUNTIME,
