/**
 * Context management for long-running agent sessions: append-only
 * history with two independent levers, replacing per-dispatch history
 * rewriting (`compactHistoryForModel`).
 *
 * Rewriting the model-bound history on every dispatch has two
 * structural costs. It caps the agent's memory horizon at the rewrite
 * window — the agent loses verbatim access to earlier work and
 * re-derives it with fresh tool calls — and it changes the prompt
 * prefix on every request, which defeats provider prompt caching
 * across turns. The levers below keep history append-only and address
 * context growth separately:
 *
 *   Lever 1 `editToolResults` — continuous and cheap: clear stale tool
 *   RESULTS (never conversation text) older than N user turns.
 *   Idempotent, so the prompt prefix stays byte-stable up to the
 *   sliding boundary and cross-turn caching survives.
 *
 *   Lever 2 compaction — a rare EVENT: when history crosses a
 *   threshold, the host runs a summarization request (model-written
 *   memory) and persists a `CompactionMarker`; the model-bound history
 *   becomes the marker summary plus everything after the boundary.
 *   `mechanicalSummary` is the fallback when the summarization call
 *   fails — a turn is never blocked on compaction.
 *
 * This module is pure functions over `ModelContextMessageLike`. The
 * host owns the summarization request, marker persistence, and any UI.
 */
import {
  type ModelContextMessageLike,
  type ModelContextToolCallLike,
...
export interface CompactionPolicy {
   ...
   * before large windows fill, so history is capped even on
   * million-token-context models. Default 150k.
   */
...
export function shouldCompact(
  ...
  const windowTerm = policy.windowTokens
    ? Math.floor(policy.windowTokens * ratio)
